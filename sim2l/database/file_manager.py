...
# Pattern that matches the IDs generated by _generate_id()
# Format: YYYYmm-ddHH-MMss-<UUID>  or a plain UUID
_VALID_ID_RE = re.compile(
    r'^(\d{6}-\d{4}-\d{6}-[0-9a-f-]{36}|[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})$',
    re.IGNORECASE,
)
...
class FileManager:
    ...
    def create_file(
        ...
        if parent_id != '0' and not self._validate_id(parent_id):
    ...
    def _validate_id(self, file_id: str) -> bool:
        """Return True only if file_id matches the expected ID format."""
        return bool(file_id and _VALID_ID_RE.match(file_id))
        ...
        if expand_children and file_data.get('is_folder', False):
            ...
            if child_ids:
                ...
                for child_id in child_ids:
                    if not self._validate_id(child_id):
                        # Skip children with invalid IDs (prevents traversal)
                        continue
                    child = self.get_file(child_id)
                    if child is not None:
                        expanded.append(child)
                file_data['objects'] = expanded
    ...
    def move_file(self, file_id: str, new_parent_id: str) -> bool:
        ...
        Returns:
            True if successful, False if file not found
        """
        if new_parent_id != '0' and not self._validate_id(new_parent_id):
            return False
