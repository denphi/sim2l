...
def _assert_execution_succeeded_or_skip(test_case: unittest.TestCase, result, context: str):
    """
    Assert notebook execution succeeded.

    In restricted environments (for example sandboxed CI), Jupyter kernels may
    be blocked from opening ports and execution fails with a permission error.
    In that case we skip instead of reporting a false regression.
    """
    test_case.assertIsNotNone(result, f"{context}: execution returned None")
    ...
    if status == "failed":
        ...
        if "operation not permitted" in lowered or "permission denied" in lowered:
            test_case.skipTest(
                f"{context}: notebook execution blocked in this environment ({error_message})"
            )
        test_case.fail(f"{context}: notebook execution failed: {error_message}")

    test_case.assertIsNotNone(
        getattr(result, "outputs", None),
        f"{context}: execution finished without outputs",
    )
...
class TestServiceManagement(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.processes = {}
        self.service_temp_dir = tempfile.mkdtemp(prefix="sim2l-service-mgmt-")
        self.ports = {
            'cache': 8011,
            'catalog': 8012,
            'results': 8013
        ...
        for name, proc in self.processes.items():
            if proc.poll() is None:  # Still running
                ...
                try:
                    ...
                    proc.kill()
        shutil.rmtree(self.service_temp_dir, ignore_errors=True)

    def start_service(self, name, module, port):
        """Start a service"""
        db_path = str(Path(self.service_temp_dir) / f"{name}_{port}.db")
...
class TestParameterizedExecution(unittest.TestCase):
    ...
    def test_run_with_default_parameters(self):
        ...
        _assert_execution_succeeded_or_skip(
            self, result, "test_run_with_default_parameters"
        )
    ...
    def test_run_with_different_parameters(self):
        ...
        for params in test_cases:
            ...
            _assert_execution_succeeded_or_skip(
                self,
                result,
                f"test_run_with_different_parameters with params={params}",
            )
...
class TestCaching(unittest.TestCase):
    ...
    def test_cache_hit(self):
        ...
        _assert_execution_succeeded_or_skip(self, result1, "test_cache_hit first run")

        # Second run - should hit cache
        start_time = time.time()
        result2 = sim.run(**params, executor=executor)
        time2 = time.time() - start_time
        _assert_execution_succeeded_or_skip(self, result2, "test_cache_hit cached run")
    ...
    def test_cache_miss_different_parameters(self):
        ...
        _assert_execution_succeeded_or_skip(self, result1, "test_cache_miss_different_parameters first run")
        _assert_execution_succeeded_or_skip(self, result2, "test_cache_miss_different_parameters second run")
...
class TestCompleteWorkflow(unittest.TestCase):
    ...
    def test_full_workflow(self):
        ...
        _assert_execution_succeeded_or_skip(self, result1, "test_full_workflow first run")

        # Step 4: Run with same parameters (should hit cache)
        start_time = time.time()
        result2 = sim.run(**params1, executor=executor)
        time2 = time.time() - start_time
        _assert_execution_succeeded_or_skip(self, result2, "test_full_workflow cached run")
        ...
        _assert_execution_succeeded_or_skip(self, result3, "test_full_workflow second parameter set")
        self.assertNotEqual(
            result1.outputs.max_temperature,
